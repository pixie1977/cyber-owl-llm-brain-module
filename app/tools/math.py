...
@tool
def math_tool(expression: str, **kwargs) -> tuple[str, bool]:
    """
    Выполняет математические вычисления.

    Обязательно используйте этот инструмент при любых числах или запросах вроде
    'синус', 'косинус', 'тангенс', 'котангенс', 'логарифм', 'плюс', 'минус', 'сколько', 'умножить'. Даже для простых задач.

    Для вычисления десятичного логарифма ЗАПРЕЩЕНО использовать log10(x). Вместо этого переводите выражение в формат log(x)/log(10) или log(x, 10)

    Example: 'cos(pi/2)' for cosine of half pi.
    degree - не использовать!
